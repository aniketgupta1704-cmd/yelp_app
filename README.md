...
Data files are generated by running the notebooks in order:

```bash
# Download raw Yelp Open Dataset from:
# https://www.yelp.com/dataset

# Then run notebooks in order — each produces artifacts for the next:
...
All artifacts land in `data/`. Once complete, the Streamlit app has everything it needs.

**Note:** Notebooks were originally built on Google Colab with GPU. Fine-tuning DistilBERT (notebook 02) takes ~15 min on a T4 GPU. Adjust batch sizes if running on CPU.
